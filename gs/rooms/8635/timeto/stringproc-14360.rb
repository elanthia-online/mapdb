...
if UserVars.mapdb_use_day_pass == true
  ...
  unless DownstreamHook.list.include?("mapdb_day_pass_monitor")
    ...
    hook_proc = proc { |s|
      ...
      elsif /quickly hands you a Chronomage day pass/.match?(s) # fixme: xml
  ...
  if (UserVars.mapdb_find_day_pass == "yes") && (bounty? !~ /^You have made contact with the child/) && ((bounty? !~ /provide a protective escort/) || (bounty? =~ /WAIT/))
    if (sack_name = UserVars.day_pass_sack)
      ...
      if sack
        ...
        elsif $mapdb_last_day_pass_message.nil? || ((Time.now - $mapdb_last_day_pass_message) > 5)
  ...
  if $mapdb_day_passes.any? { |id, h| h[:towns].include?("Wehnimer's Landing") && h[:towns].include?("Solhaven") && (h[:expires] > (Time.now + 10)) }
    0.8
  elsif /^(yes|true)$|\bwl,sol\b/i.match?(UserVars.mapdb_buy_day_pass.to_s)
    4.4
  end
end
