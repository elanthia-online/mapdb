...
if UserVars.mapdb_use_day_pass == true
  ...
  if $mapdb_day_passes.any? { |id, h| h[:towns].include?("Solhaven") && h[:towns].include?("Icemule Trace") && (h[:expires] > (Time.now + 10)) }
    0.8
  elsif /^(yes|true)$|\bimt,sol\b/i.match?(UserVars.mapdb_buy_day_pass.to_s)
    8.8
  end
end
