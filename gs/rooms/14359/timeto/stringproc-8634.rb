...
if UserVars.mapdb_use_day_pass == true
  ...
  if $mapdb_day_passes.any? { |id, h| h[:towns].include?("Solhaven") && h[:towns].include?("Wehnimer's Landing") && (h[:expires] > (Time.now + 10)) }
    0.8
  elsif /^(yes|true)$|\bsol,wl\b/i.match?(UserVars.mapdb_buy_day_pass.to_s)
    7.4
  end
end
