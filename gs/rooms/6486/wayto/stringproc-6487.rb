if !/^Obvious/.match?(dothistimeout("go stone doors", 3, /^Obvious exits: west$|^You cannot/))
  s = Spell[920]
  if s.known?
    if s.active?
    ...
    g = proc { |l|
      l.split(",").each { |d|
        r = dothistimeout(("tell familiar to go " + d), 15, /^You sense|just squeezed between the stone doors|^Obvious (?:exits|paths)/)
        if r.nil? || (r =~ /^You sense confusion/)
          fail = true
          next
    ...
    4.times { |i|
      i.zero? ? g.call("doors,w,s,arch,ne,e,privy,hole,w") : g.call("nw,w,w,n,")
      break if fail
