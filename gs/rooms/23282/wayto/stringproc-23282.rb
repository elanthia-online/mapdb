...
$mapdb_confluence_wayto ||= {}
$mapdb_confluence_wander ||= []
loop {
  start_room = Room.current
  break if start_room.id == $mapdb_confluence_target
  if hot_rooms.include?(start_room.id)
    hot = true; elsif cold_rooms.include?(start_room.id); hot = false; else
                                                                         $go2_restart = true
                                                                         break
  end
  if GameObj.loot.any? { |o| o.name == "point of elemental tranquility" }
    hot ? ($mapdb_confluence_hot_tranquility = start_room.id) : ($mapdb_confluence_cold_tranquility = start_room.id)
    ...
    $mapdb_confluence_cold_tranquility = nil
  end; if GameObj.loot.any? { |o| o.name == "gaping bottomless pit" }
         hot ? ($mapdb_confluence_hot_pit = start_room.id) : ($mapdb_confluence_cold_pit = start_room.id)
       ...
       end; if $mapdb_confluence_wayto[start_room.id].nil?
              wayto = {}
  ...
  dir_to = proc { |targets|
    ...
    tried = []
    30.times {
      break if dir = $mapdb_confluence_wayto[start_room.id].keys.find { |d| targets.include?($mapdb_confluence_wayto[start_room.id][d]) }
      targets.each { |i| tried.push(i) unless tried.include?(i) }
      ot = targets
      targets = $mapdb_confluence_wayto.keys.find_all { |k| $mapdb_confluence_wayto[k].values.any? { |i| ot.include?(i) } and !tried.include?(k) }
  ...
  if $mapdb_confluence_target == "tranquility"
    ...
    if hot && $mapdb_confluence_hot_tranquility
      dir = dir_to.call([$mapdb_confluence_hot_tranquility])
    elsif !hot && $mapdb_confluence_cold_tranquility
      dir = dir_to.call([$mapdb_confluence_cold_tranquility])
    end; else
           if hot && cold_rooms.include?($mapdb_confluence_target); if GameObj.loot.any? { |o| o.name == "gaping bottomless pit" }
                                                                      move "go pit"
                                                                      redo
                                                                    end
                                                                    dir = dir_to.call([$mapdb_confluence_hot_pit]) if $mapdb_confluence_hot_pit
           elsif !hot && hot_rooms.include?($mapdb_confluence_target)
  ...
  dir ||= dir_to.call([nil])
  dir ||= $mapdb_confluence_wayto[start_room.id].keys.find { |d| !$mapdb_confluence_wander.include?($mapdb_confluence_wayto[start_room.id][d]) }
  if dir.nil?
    next_id = $mapdb_confluence_wander.find { |i| $mapdb_confluence_wayto[start_room.id].value?(i) }
